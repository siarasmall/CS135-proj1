...
def calc_mean_binary_cross_entropy_from_probas(ytrue_N, yproba1_N):
    ...
    bce_prob = 0.0
    bce_prob = - ytrue_N * np.log2(yproba1_N) - (1- ytrue_N) * np.log2(1 - yproba1_N)  # TODO fix me
    mbce = 1/(N+1e-100) * np.sum(bce_prob)
    
    return mbce  # TODO fix me
...
def calc_mean_binary_cross_entropy_from_scores(ytrue_N, scores_N):
    ...
    flipped_scores_N = scores_N * yflippedsign_N
    flipped_scores_N = flipped_scores_N.reshape(N, 1)

    scores_and_zeros_N2 = np.hstack([ np.zeros((N, 1)),flipped_scores_N]) 
    ...
    mbce = 1/(N+1e-100) * np.sum(bce_score)
